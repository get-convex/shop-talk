      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {lists.map(list => (
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-semibold text-amber-800">{list.name}</h2>
            </div>
            ...
            <div className="space-y-2">
              {list.items.slice(0, 3).map(item => (
                ...
                >
                  <div className={`w-2 h-2 rounded-full ${item.completed ? "bg-green-500" : "bg-amber-500"}`} />
        ...
        {lists.length === 0 && !isCreating && (
          <div className="col-span-full text-center py-12 bg-amber-50 rounded-lg border-2 border-dashed border-amber-200">
            ...
            <h3 className="text-xl font-medium text-amber-800 mb-2">No Shopping Lists Yet</h3>
            <p className="text-amber-600 mb-4">Create your first shopping list to get started</p>
            <Button 
              onClick={() => setIsCreating(true)}
              className="bg-amber-500 hover:bg-amber-600 text-white"
            >
